...
print("--- Computing Weighted Average (Optimized) with 10-Fold CV ---")
...
# Pre-fetch model probs: (n_samples, n_models, 3)
...
for train_idx, test_idx in skf.split(np.zeros(len(merged_df)), y_true):
    ...
    def loss_func_fold(weights):
        ...
        if np.sum(weights) == 0: return 100
        weights /= np.sum(weights)
        # Broadcast weights: (1, n_models, 1)
...
print(f"Weighted Average Mean Accuracy: {np.mean(wa_scores):.4f} (+/- {np.std(wa_scores):.4f})")
weighted_probs = weighted_probs_cv
y_pred_weighted = np.argmax(weighted_probs, axis=1)

# D. Stacking (Linear)
print("--- Computing Stacking (Logistic) with 10-Fold CV ---")
...
skf = StratifiedKFold(n_splits=10, shuffle=True, random_state=42)
cv_scores_lr = cross_val_score(meta_model, X_stack, y_true, cv=skf, scoring='accuracy')
print(f"Stacking (LogReg) Mean Accuracy: {cv_scores_lr.mean():.4f} (+/- {cv_scores_lr.std():.4f})")
stacking_probs = cross_val_predict(meta_model, X_stack, y_true, cv=skf, method='predict_proba')
...
print("--- Computing Stacking (Random Forest) with 10-Fold CV ---")
rf_meta = RandomForestClassifier(n_estimators=50, max_depth=10, random_state=42)

cv_scores_rf = cross_val_score(rf_meta, X_stack, y_true, cv=skf, scoring='accuracy')
print(f"Stacking (RandomForest) Mean Accuracy: {cv_scores_rf.mean():.4f} (+/- {cv_scores_rf.std():.4f})")
rf_probs = cross_val_predict(rf_meta, X_stack, y_true, cv=skf, method='predict_proba')
y_pred_rf = np.argmax(rf_probs, axis=1)

# H. Stacking (Gradient Boosting - Non-Linear)
print("--- Computing Stacking (Gradient Boosting) with 10-Fold CV ---")
gb_meta = GradientBoostingClassifier(n_estimators=116, max_depth=6, random_state=42)

cv_scores_gb = cross_val_score(gb_meta, X_stack, y_true, cv=skf, scoring='accuracy')
print(f"Stacking (GradBoost) Mean Accuracy: {cv_scores_gb.mean():.4f} (+/- {cv_scores_gb.std():.4f})")
gb_probs = cross_val_predict(gb_meta, X_stack, y_true, cv=skf, method='predict_proba')
...
print(f"Best performing model is '{best_model_name}' based on F1-Score.")
