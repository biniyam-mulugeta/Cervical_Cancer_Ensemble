...
try:
    import cuml
    from cuml.linear_model import LogisticRegression as LR_GPU
    from cuml.ensemble import RandomForestClassifier as RF_GPU
    CUML_AVAILABLE = True
    print("RAPIDS cuML detected. Using GPU for Logistic Regression and Random Forest.")
except ImportError:
    CUML_AVAILABLE = False
    print("RAPIDS cuML not found. Using CPU for Logistic Regression and Random Forest.")

try:
    from xgboost import XGBClassifier
    XGB_AVAILABLE = True
    print("XGBoost detected. Using GPU for Gradient Boosting.")
except ImportError:
    XGB_AVAILABLE = False
    print("XGBoost not found. Using CPU for Gradient Boosting.")
...
# Define crossvalidation folds to run
CV_FOLDS = [3,5, 10]

# Storage for CV Statistics
cv_stats_list = []

def add_cv_stats(model_name, k, acc, f1, prec, rec):
    cv_stats_list.append({
        "Model": model_name,
        "K-Folds": k,
        "Mean Accuracy": np.mean(acc),
        "Std Dev Accuracy": np.std(acc),
        "Min Accuracy": np.min(acc),
        "Max Accuracy": np.max(acc),
        "Mean F1-Score": np.mean(f1),
        "Mean Precision": np.mean(prec),
        "Mean Recall": np.mean(rec)
    })
...
# --- NEW: Calculate CV Statistics for Non-Trainable Ensembles ---
for k in CV_FOLDS:
    print(f"--- Evaluating Non-Trainable Ensembles with {k}-Fold CV ---")
    skf = StratifiedKFold(n_splits=k, shuffle=True, random_state=42)
    methods = ["Simple Average", "Hard Voting", "Rank Averaging", "Geometric Mean"]
    metrics = {m: {'acc': [], 'f1': [], 'prec': [], 'rec': []} for m in methods}

    for _, test_idx in skf.split(np.zeros(len(merged_df)), y_true):
        X_fold = X_all[test_idx] # Shape: (n_samples_fold, n_models, 3)
        y_fold = y_true[test_idx]
        
        # 1. Simple Average CV
        probs_fold_avg = np.mean(X_fold, axis=1)
        pred_avg = np.argmax(probs_fold_avg, axis=1)

        # 2. Hard Voting CV
        votes_fold = np.zeros((len(y_fold), 3))
        for m_idx in range(len(models)):
            preds_m = np.argmax(X_fold[:, m_idx, :], axis=1)
            for i, p in enumerate(preds_m):
                votes_fold[i, p] += 1
        pred_vote = np.argmax(votes_fold, axis=1)

        # 3. Rank Averaging CV
        rank_accum_fold = np.zeros((len(y_fold), 3))
        for m_idx in range(len(models)):
            for c in range(3):
                rank_accum_fold[:, c] += rankdata(X_fold[:, m_idx, c])
        pred_rank = np.argmax(rank_accum_fold, axis=1)

        # 4. Geometric Mean CV
        # Move axis 1 (models) to last for gmean: (n_samples, 3, n_models) -> gmean over axis 2
        gmean_fold = gmean(np.moveaxis(X_fold, 1, 2), axis=2)
        pred_gmean = np.argmax(gmean_fold, axis=1)
        
        preds_map = {
            "Simple Average": pred_avg,
            "Hard Voting": pred_vote,
            "Rank Averaging": pred_rank,
            "Geometric Mean": pred_gmean
        }

        for m in methods:
            y_p = preds_map[m]
            metrics[m]['acc'].append(accuracy_score(y_fold, y_p))
            metrics[m]['f1'].append(f1_score(y_fold, y_p, average='weighted'))
            metrics[m]['prec'].append(precision_score(y_fold, y_p, average='weighted', zero_division=0))
            metrics[m]['rec'].append(recall_score(y_fold, y_p, average='weighted', zero_division=0))

    for m in methods:
        add_cv_stats(m, k, metrics[m]['acc'], metrics[m]['f1'], metrics[m]['prec'], metrics[m]['rec'])

for k in CV_FOLDS:
    print(f"--- Computing Weighted Average (Optimized) with {k}-Fold CV ---")
    weighted_probs_cv = np.zeros((len(merged_df), 3))
    skf = StratifiedKFold(n_splits=k, shuffle=True, random_state=42)
    wa_acc, wa_f1, wa_prec, wa_rec = [], [], [], []
    ...
    for train_idx, test_idx in skf.split(np.zeros(len(merged_df)), y_true):
        ...
        wa_acc.append(accuracy_score(y_test, y_pred_fold))
        wa_f1.append(f1_score(y_test, y_pred_fold, average='weighted'))
        wa_prec.append(precision_score(y_test, y_pred_fold, average='weighted', zero_division=0))
        wa_rec.append(recall_score(y_test, y_pred_fold, average='weighted', zero_division=0))

    add_cv_stats("Weighted Average", k, wa_acc, wa_f1, wa_prec, wa_rec)
...
scoring = {'acc': 'accuracy', 'f1': 'f1_weighted', 'prec': 'precision_weighted', 'rec': 'recall_weighted'}

for k in CV_FOLDS:
    print(f"--- Computing Stacking (Logistic) with {k}-Fold CV ---")
    skf = StratifiedKFold(n_splits=k, shuffle=True, random_state=42)
    scores = cross_validate(meta_model_lr, X_stack, y_true, cv=skf, scoring=scoring)
    add_cv_stats("Stacking (LogReg)", k, scores['test_acc'], scores['test_f1'], scores['test_prec'], scores['test_rec'])
...
stacking_rf_preds = {}
stacking_rf_probs = {}
rf_meta = RandomForestClassifier(n_estimators=150, max_depth=15, random_state=42)

for k in CV_FOLDS:
    print(f"--- Computing Stacking (Random Forest) with {k}-Fold CV ---")
    skf = StratifiedKFold(n_splits=k, shuffle=True, random_state=42)
    scores = cross_validate(rf_meta, X_stack, y_true, cv=skf, scoring=scoring)
    add_cv_stats("Stacking (RandomForest)", k, scores['test_acc'], scores['test_f1'], scores['test_prec'], scores['test_rec'])
...
stacking_gb_preds = {}
stacking_gb_probs = {}
gb_meta = GradientBoostingClassifier(n_estimators=100, max_depth=15, random_state=42)

for k in CV_FOLDS:
    print(f"--- Computing Stacking (Gradient Boosting) with {k}-Fold CV ---")
    skf = StratifiedKFold(n_splits=k, shuffle=True, random_state=42)
    scores = cross_validate(gb_meta, X_stack, y_true, cv=skf, scoring=scoring)
    add_cv_stats("Stacking (GradBoost)", k, scores['test_acc'], scores['test_f1'], scores['test_prec'], scores['test_rec'])
